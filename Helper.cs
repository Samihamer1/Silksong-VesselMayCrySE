...
namespace VesselMayCrySE
{
    internal static class Helper
    {
        ...
        /// <summary>
        /// Starting from GameObject go, traverses children searching in sequence using childNames
        /// Example: gameObject.ChildChain("childOfGameObject", "childOfChildOfGameObject);
        /// </summary>
        /// <param name="go"></param>
        /// <param name="childNames"></param>
        /// <returns>The final GameObject, or null if the chain fails</returns>
        public static GameObject? ChildChain(this GameObject go, params string[] childNames)
        {
            if (go == null) { return null; }

            GameObject? currentObject = go;
            foreach (string childName in childNames)
            {
                currentObject = currentObject.Child(childName);
        ...
        {
            if (HeroController.instance.gameObject == null) { VesselMayCrySEPlugin.Instance.LogError("VMCSE GetHornetOwnerDefault should only be called when Hornet is active"); return null; }

            FsmOwnerDefault hornetOwnerDefault = new FsmOwnerDefault { gameObject = HeroController.instance.gameObject, ownerOption = OwnerDefaultOption.SpecifyGameObject };
