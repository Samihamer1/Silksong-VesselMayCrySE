    ...
    public class DevilCrestHandler : MonoBehaviour
    {
        ...
        private void SetupChaserBlades()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (sword == null)
                {
                    VesselMayCrySEPlugin.Instance.LogError("Chaser blade not properly created!");
                    break;
                }
        ...
        private void PatchBind()
        {
            ...
            if (BindTypeState == null) { VesselMayCrySEPlugin.Instance.LogError("Bind Type state not found in Spell Control. Incompatibility with another mod?"); return; }
            if (BindBurstState == null) { VesselMayCrySEPlugin.Instance.LogError("Bind Burst state not found in Spell Control. Incompatibility with another mod?"); return; }
