...
using VesselMayCrySE.CanvasHandler;
...
namespace VesselMayCrySE.Components
{
    public class DevilTrigger : MonoBehaviour
    {
        ...
        private float TRIGGERGAIN = 10f;
        private float trigger = 0f;

        private float triggerHealTimer = 0;
        private float TRIGGERHEALTIME = 4.5f;
        ...
        private void CreateTriggerEffect()
        {
            ...
            if (hornet == null) { VesselMayCrySEPlugin.Instance.LogError("Hornet not found when creating trigger effect. Called too early?");  return;  }
        ...
        private void Update()
        {
            ...
            //trigger heal
            triggerHealTimer += Time.deltaTime;
            if (triggerHealTimer >= TRIGGERHEALTIME)
            {
                triggerHealTimer = 0;
